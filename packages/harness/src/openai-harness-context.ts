...
type BuildResponsesHistoryInputArgs = {
  config?: Pick<HarnessConfig, 'LLM_MAX_OUTPUT_TOKENS' | 'MODEL_CONTEXT_WINDOW_TOKENS' | 'MODEL_AUTO_COMPACT_TOKEN_LIMIT'>;
  history: StoredEvent[];
...
export const resolveContextWindowTokens = (
  config?: Pick<HarnessConfig, 'MODEL_CONTEXT_WINDOW_TOKENS'>,
) => config?.MODEL_CONTEXT_WINDOW_TOKENS ?? DEFAULT_MODEL_CONTEXT_WINDOW_TOKENS;

export const resolveContextBudgetTokens = (
  config?: Pick<HarnessConfig, 'LLM_MAX_OUTPUT_TOKENS' | 'MODEL_CONTEXT_WINDOW_TOKENS'>,
) => {
