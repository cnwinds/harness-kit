...
const resolveEnabledSkillScriptPath = (scriptPath: string, availableSkills: SkillDescriptor[]) => {
  ...
  const manifest = matchedSkill.manifest as {
    permissions?: { scripts?: boolean };
    runtime?: { type?: string; entrypoints?: { path: string }[] };
  } | undefined;

  if (
    matchedSkill.source === 'installed'
    && (
      !manifest?.permissions?.scripts
      || manifest.runtime?.type === 'none'
    )
  ) {
    throw new Error(`Skill does not allow script execution: ${matchedSkill.name}`);
  }

  if (matchedSkill.source === 'installed' && manifest?.runtime?.entrypoints) {
    const allowedEntrypoints = new Set(
      manifest.runtime.entrypoints.map((entrypoint: { path: string }) => normalizeWorkspaceToolPath(entrypoint.path)),
    );
