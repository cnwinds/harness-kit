...
export const ensureBaseDirectories = async (config: HarnessConfig) => {
  await fs.mkdir(config.DATA_ROOT, { recursive: true });
  if (config.INSTALLED_SKILLS_ROOT) {
    await fs.mkdir(config.INSTALLED_SKILLS_ROOT, { recursive: true });
  }
  await fs.mkdir(getUsersRoot(config), { recursive: true });
};

export const ensureUserDirectories = async (config: HarnessConfig, userId: string) => {
