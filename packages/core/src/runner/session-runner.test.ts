...
import { defaultHarnessConfig, type HarnessConfig } from '../config.js';
import { ensureBaseDirectories, ensureSessionDirectories, ensureUserDirectories } from '../storage/fs-utils.js';
import { SessionRunner } from './session-runner.js';

const createConfig = (rootDir: string): HarnessConfig => defaultHarnessConfig({
  CWD: rootDir,
...
describe('SessionRunner', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'harness-runner-test-'));
